...
describe("createOAuthState / verifyOAuthState", () => {
  it("round-trips a valid state and recovers the payload", () => {
    const state = createOAuthState({ practiceId: "practice-1", dentistId: "dentist-1" }, SECRET);
    const result = verifyOAuthState(state, SECRET);
    expect(result.valid).toBe(true);
    expect(result.payload).toMatchObject({ practiceId: "practice-1", dentistId: "dentist-1" });
  });

  it("supports a null dentistId (practice-wide connection)", () => {
    const state = createOAuthState({ practiceId: "practice-1", dentistId: null }, SECRET);
    const result = verifyOAuthState(state, SECRET);
    expect(result.payload?.dentistId).toBeNull();
  });
  ...
  it("rejects a tampered payload even reusing the original signature", () => {
    ...
    const tamperedPayload = Buffer.from(
      JSON.stringify({ practiceId: "someone-elses-practice", dentistId: null, nonce: "x", issuedAt: Date.now() }),
      "utf8"
    ).toString("base64url");
