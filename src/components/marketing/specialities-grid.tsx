      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <RevealGroup className="mt-10 flex flex-wrap gap-2.5">
          {SPECIALITIES.map(({ name, icon: Icon }) => (
            <RevealItem key={name} className="inline-block">
              <span className="inline-flex items-center gap-2 rounded-full border border-border bg-card px-4 py-2 text-sm text-foreground shadow-xs transition-all duration-base hover:-translate-y-0.5 hover:border-primary/30 hover:shadow-md">
                <Icon className="size-3.5 text-primary" />
                {name}
              </span>
            </RevealItem>
          ))}
          <RevealItem className="inline-block">
